...
public class FinalizarVenda extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int idTipoDePagamento = Integer.parseInt(request.getParameter("id"));
            try{
                Venda v = (Venda) session.getAttribute("venda");
                TipoDePagamentoDAO tpDAO = new TipoDePagamentoDAO();
                v.setTipoDePagamento(tpDAO.carregarPorId(idTipoDePagamento));
                VendaDAO vDAO = new VendaDAO();
                int id = vDAO.inserir(v);
                ProdutoDAO pDAO = new ProdutoDAO();
                pDAO.baixaNoEstoque(v.getCarrinho());
